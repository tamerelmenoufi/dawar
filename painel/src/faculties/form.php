<?php
    ...
    if($_POST['action'] == 'save'){
        ...
        if($_POST['id']){
            $query = "update faculties set {$attr} where id = '{$_POST['id']}'";
            mysqli_query($con, $query);
            $id = $_POST['id'];
        }else{
            $query = "insert into faculties set {$attr}";
            mysqli_query($con, $query);
    ...
    $query = "select * from faculties where id = '{$_POST['id']}'";
...
<h4 class="Titulo<?=$md5?>"><?=$Dic['Faculty Registration']?></h4>
    <form id="form-<?= $md5 ?>">
        <div class="row">
            <div class="col">
                <div class="form-floating mb-3">
                    <input type="text" class="form-control" id="faculty" name="faculty" placeholder="<?=$Dic['Faculty']?>" value="<?=$d->faculty?>">
                    <label for="faculty"><?=$Dic['Faculty']?>*</label>
                </div>
                <div class="form-floating mb-3">
                    <select name="state" id="state" class="form-control" placeholder="<?=$Dic['States']?>">
                        <option value="">::<?=$Dic['States']?>::</option>
                        <?php
                            $q = "select * from states order by state";
                            $r = mysqli_query($con, $q);
                            while($s = mysqli_fetch_object($r)){
                        ?>
                        <option value="<?=$s->id?>" <?=(($d->state == $s->id)?'selected':false)?>><?=$s->state?></option>
                        <?php
                            }
                        ?>
                    </select>
